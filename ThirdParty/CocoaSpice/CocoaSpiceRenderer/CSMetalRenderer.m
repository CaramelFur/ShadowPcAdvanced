...
- (nonnull instancetype)initWithMetalKitView:(nonnull MTKView *)mtkView
{
    ...
    if(self)
    {
        ...
        // FunkyShadow: CSShaders.metal is compiled into the app's default.metallib.
...
- (void)_setViewportCGSize:(CGSize)size {
    ...
    // FunkyShadow: a resized drawable must be redrawn even when the scale is
    // unchanged (letterboxed), or the previous frame is shown stretched.
...
- (void)drawInMTKView:(nonnull MTKView *)view
{
    ...
    if (renderPassDescriptor == nil || currentDrawable == nil) {
        // FunkyShadow: no drawable (minimised, off screen) must not leave the
        // SPICE thread waiting for a draw that never comes.
        [self _completeDraw];
        return;
    }
...
- (void)disableRender {
    dispatch_async(dispatch_get_main_queue(), ^{
        ...
        self.renderNeedsUpdate = NO;
        // FunkyShadow: the view may never draw again (window closing); anyone
        // waiting on a draw has to be let go now.
        [self _completeDraw];
    });
}
...
- (BOOL)_renderCommand:(id<MTLCommandBuffer>)commandBuffer
  ...
  renderPassDescriptor:(MTLRenderPassDescriptor *)renderPassDescriptor {
    ...
    return YES; // FunkyShadow: upstream falls off the end (an error under Xcode's defaults)
}
